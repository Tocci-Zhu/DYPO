...
from verl.trainer.ppo.ray_trainer_gspo import RayPPOTrainer
...
@hydra.main(config_path="config", config_name="ppo_trainer_new", version_base=None)
def main(config):
    run_ppo(config)
...
class TaskRunner:
    def run(self, config):
        ...
        from verl.trainer.ppo.ray_trainer_gspo import Role
        ...
        train_dataset = create_rl_dataset(config.data.train_files, config.data, tokenizer, processor, enable_rag=False)
        val_dataset = create_rl_dataset(config.data.val_files, config.data, tokenizer, processor, enable_rag=False)
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, enable_rag=True):
