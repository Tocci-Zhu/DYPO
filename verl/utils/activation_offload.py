    ...
    def offload(src_tensor, pin_memory=True):
        ...
        if src_tensor.numel() == 0:
            cpu_backup = torch.empty(
                ...
                device="cpu",
                pin_memory=False,  # Don't use pinned memory for empty tensors
            )
        ...
        try:
            ...
            # Fallback to non-pinned memory if pinned memory allocation fails
            logger.warning(f"Failed to allocate pinned memory for tensor of shape {src_tensor.size()}, "
                          f"falling back to non-pinned memory. Error: {e}")
            cpu_backup = torch.empty(
                src_tensor.size(),
                dtype=src_tensor.dtype,
                device="cpu",
        ...
        cpu_backup.copy_(src_tensor, non_blocking=cpu_backup.is_pinned())
        state = (src_tensor.device, cpu_backup)
        return state
