...
from verl.utils.dataset.rl_dataset_dypo import RLHFDataset
...
class RLHFDatasetWithTarget(RLHFDataset):
    ...
    def __getitem__(self, item):
        ...
        if tgt is not None and sample is True:
            ...
            tgt_input_ids = self.tokenizer(tgt['content'][idx], add_special_tokens=False, return_tensors='pt')['input_ids'].reshape(-1) # [1, l]
        ...
        # 计算 target 的 attention mask（非 pad token 为 1，pad token 为 0）
        tgt_attention_mask = (tgt_input_ids != self.tokenizer.pad_token_id).long()

        row_dict['tgt_input_ids'] = tgt_input_ids
        row_dict['tgt_attention_mask'] = tgt_attention_mask
