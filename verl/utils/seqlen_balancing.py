...
def rearrange_micro_batches(
    ...
    # 保护性检查：确保微批次数量不超过批次大小
    if num_micro_batches > len(seq_len_effective):
        print(f"[Warning] num_micro_batches ({num_micro_batches}) > len(seq_len_effective) ({len(seq_len_effective)}). "
              f"Adjusting num_micro_batches to {len(seq_len_effective)}")
        num_micro_batches = len(seq_len_effective)
