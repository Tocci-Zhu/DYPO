        ...
        for epoch_idx in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # 先定义 on_policy_mask，后续对比学习需要用到
                    on_policy_mask = None
                    
                    if self.config.offline_loss_type == "sft":
                        # 原始的 off_policy 判断（基于是否有 prefix）
                        has_prefix_mask = data['prefix_mask'].any(-1) # [bsz]
                        
                        # 应用 whether_pad mask：只考虑有效样本
                        if has_padding and 'whether_pad' in data:
                            sample_valid_mask = ~data['whether_pad']  # [bsz]
                            # off_policy: 有 prefix 且是有效样本
                            off_policy_mask = has_prefix_mask & sample_valid_mask
                            # on_policy: 没有 prefix 且是有效样本
                            on_policy_mask = (~has_prefix_mask) & sample_valid_mask
                        else:
                            off_policy_mask = has_prefix_mask
                            on_policy_mask = ~has_prefix_mask
                        
                        off_policy_logprob = log_prob[off_policy_mask]
                        off_policy_eos_mask = response_mask[off_policy_mask]
                        
                        # Check if we have off-policy data for SFT loss
                        if off_policy_logprob.numel() > 0 and off_policy_eos_mask.sum() > 0:
                            sft_loss = compute_sft_pure_loss(log_prob=off_policy_logprob,
                                                            response_mask=off_policy_eos_mask)
                            print(f"[SFT Debug] sft_loss: {sft_loss.item()}")
                        else:
                            # No off-policy data, initialize sft_loss as NaN
                            device = log_prob.device
                            sft_loss = torch.tensor(float('nan'), device=device)
                            print(f"[SFT Debug] No off-policy data, sft_loss set to NaN")
                        
                        print(f"[SFT Debug] sft_loss_coef: {self.config.sft_loss_coef}")

                        on_policy_logprob = log_prob[on_policy_mask]
                        on_policy_old_logprob = old_log_prob[on_policy_mask]
                        
                        n_valid = sample_valid_mask.sum().item() if (has_padding and 'whether_pad' in data) else on_policy_mask.size(0)
                        ...
                        # Check if we have on-policy samples for PG loss
                        has_on_policy_data = (on_policy_logprob.numel() > 0 and on_policy_eos_mask.sum() > 0)
                        
                        # Check if we have valid SFT loss
                        has_sft_data = not torch.isnan(sft_loss)
                        
                        # Calculate sample ratios for dynamic loss scaling
                        n_on_policy = on_policy_mask.sum().item()
                        n_off_policy = off_policy_mask.sum().item() if has_sft_data else 0
                        total_valid_samples = n_on_policy + n_off_policy
                        
                        # Compute dynamic weights based on sample ratios
                        if total_valid_samples > 0:
                            rl_ratio = n_on_policy / total_valid_samples
                            sft_ratio = n_off_policy / total_valid_samples
                        else:
                            rl_ratio = 0.0
                            sft_ratio = 0.0
                        
                        # Apply smoothing to avoid zero weights (optional)
                        weight_smooth_alpha = self.config.get('weight_smooth_alpha', 0.0)
                        if weight_smooth_alpha > 0:
                            rl_weight = max(rl_ratio, weight_smooth_alpha) if n_on_policy > 0 else 0.0
                            sft_weight = max(sft_ratio, weight_smooth_alpha) if n_off_policy > 0 else 0.0
                        else:
                            # No smoothing: use ratios directly (default to 1.0 when only one type exists)
                            rl_weight = rl_ratio if n_off_policy > 0 else 1.0
                            sft_weight = sft_ratio if n_on_policy > 0 else 1.0
                        
                        print(f"[Loss Fusion] Sample ratios - RL: {n_on_policy}/{total_valid_samples} ({rl_ratio:.3f}), "
                              f"SFT: {n_off_policy}/{total_valid_samples} ({sft_ratio:.3f})")
                        print(f"[Loss Fusion] Dynamic weights - RL: {rl_weight:.3f}, SFT: {sft_weight:.3f}")
                        
                        # Initialize policy_loss as None, will be set by first valid loss
                        device = advantages.device if advantages.numel() > 0 else torch.device('cuda')
                        policy_loss = None
                        
                        # Compute PG loss if we have on-policy data
                        if has_on_policy_data:
                            ...
                            # Apply dynamic weight to PG loss
                            weighted_pg_loss = rl_weight * pg_loss
                            print(f"[Loss Fusion] pg_loss: {pg_loss.item():.6f}, weighted: {weighted_pg_loss.item():.6f} (weight: {rl_weight:.3f})")
                            
                            # Initialize or accumulate policy_loss
                            if policy_loss is None:
                                policy_loss = weighted_pg_loss
                            else:
                                policy_loss = policy_loss + weighted_pg_loss
                            
                            # Record RL metrics
                            micro_batch_metrics['actor/rl_weight'] = rl_weight
                            micro_batch_metrics['actor/n_rl_samples'] = n_on_policy
                        else:
                            # No on-policy data, set dummy values for metrics
                            pg_loss = torch.tensor(0.0, device=device)
                            pg_clipfrac = torch.tensor(0.0, device=device)
                            ppo_kl = torch.tensor(0.0, device=device)
                            print(f"[Loss Fusion] No on-policy data for PG loss")
                            
                            micro_batch_metrics['actor/n_rl_samples'] = 0
                            
                            # Use KL divergence as substitute when no RL data but have SFT data
                            use_kl_substitute = self.config.get('use_kl_substitute', True)
                            if has_sft_data and use_kl_substitute:
                                # Compute KL divergence to constrain policy from drifting
                                ref_log_prob = data["old_log_probs"][off_policy_mask]
                                kl_substitute_loss = compute_kl_divergence_loss(
                                    log_prob=off_policy_logprob,
                                    ref_log_prob=ref_log_prob,
                                    response_mask=off_policy_eos_mask,
                                    loss_agg_mode=loss_agg_mode
                                )
                                
                                kl_substitute_coef = self.config.get('kl_substitute_coef', 0.01)
                                weighted_kl_loss = kl_substitute_coef * kl_substitute_loss
                                ...
                                print(f"[Loss Fusion] Using KL substitute: {kl_substitute_loss.item():.6f}, "
                                      f"weighted: {weighted_kl_loss.item():.6f} (coef: {kl_substitute_coef})")
                                
                                # Record KL substitute metrics
                                micro_batch_metrics['actor/kl_substitute_loss'] = kl_substitute_loss.detach().item()
                                micro_batch_metrics['actor/kl_substitute_coef'] = kl_substitute_coef
                            else:
                                micro_batch_metrics['actor/kl_substitute_loss'] = 0.0
                        
                        # Add SFT loss if we have valid SFT data
                        if has_sft_data:
                            # Apply dynamic weight to SFT loss
                            weighted_sft_loss = sft_weight * sft_loss * self.config.sft_loss_coef
                            print(f"[Loss Fusion] sft_loss: {sft_loss.item():.6f}, weighted: {weighted_sft_loss.item():.6f} "
                                  f"(weight: {sft_weight:.3f}, coef: {self.config.sft_loss_coef})")
                            
                            # Initialize or accumulate policy_loss
                            if policy_loss is None:
                                policy_loss = weighted_sft_loss
                            else:
                                policy_loss = policy_loss + weighted_sft_loss
                            
                            # Record SFT metrics
                            micro_batch_metrics['actor/sft_loss'] = sft_loss.detach().item()
                            micro_batch_metrics['actor/sft_loss_weighted'] = weighted_sft_loss.detach().item()
                            micro_batch_metrics['actor/sft_weight'] = sft_weight
                            micro_batch_metrics['actor/sft_loss_coef'] = self.config.sft_loss_coef
                            micro_batch_metrics['actor/n_sft_samples'] = n_off_policy
                        else:
                            print('[Loss Fusion] No valid SFT data, skipping sft_loss')
                            micro_batch_metrics['actor/sft_loss'] = 0.0
                            micro_batch_metrics['actor/n_sft_samples'] = 0
                        
                        # Handle entropy ONLY if we have valid policy loss
                        # This ensures we don't create gradients when there's no valid data
                        if policy_loss is not None and entropy_coeff != 0:
                            ...
                            # Subtract entropy loss (encourage exploration)
                            policy_loss = policy_loss - entropy_loss * entropy_coeff
                            metrics['actor/entropy_coeff'] = entropy_coeff
                            print(f"[Loss Fusion] entropy_loss: {entropy_loss.item():.6f}, entropy_coeff: {entropy_coeff:.6f}")

                        # Record sample ratio metrics
                        micro_batch_metrics['actor/sample_ratio_rl'] = rl_ratio if total_valid_samples > 0 else 0.0
                        micro_batch_metrics['actor/sample_ratio_sft'] = sft_ratio if total_valid_samples > 0 else 0.0
                        micro_batch_metrics['actor/total_valid_samples'] = total_valid_samples
                        
                        # Record final policy loss (before entropy)
                        if policy_loss.requires_grad:
                            micro_batch_metrics['actor/policy_loss_fusion'] = policy_loss.detach().item()
                        
                        # Safety check: if no valid loss at all, create a zero loss (should be extremely rare)
                        # This prevents backward errors, but ideally this should never happen
                        if policy_loss is None:
                            print('[Loss Fusion] WARNING: No valid loss computed, skipping this micro-batch')
                            # Create a dummy loss without requires_grad to skip backward safely
                            continue
