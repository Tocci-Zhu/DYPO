...
def convert_dtensor_to_tensor(param):
    """Convert DTensor to regular tensor safely without using full_tensor().
    
    This avoids NCCL backend issues with allgather_into_tensor_coalesced.
    
    Args:
        param: Parameter tensor (can be DTensor or regular tensor)
        
    Returns:
        Regular tensor (not DTensor)
    """
    if isinstance(param, DTensor):
        # Check if the DTensor is already replicated
        from torch.distributed.tensor.placement_types import Replicate
        placements = param.placements
        is_replicated = all(isinstance(p, Replicate) for p in placements)
        
        if is_replicated:
            # If already replicated, just get the local tensor
            return param.to_local()
        else:
            # For sharded tensors, manually gather using all_gather
            # This avoids the NCCL backend issue with full_tensor()
            local_tensor = param.to_local()
            mesh = param.device_mesh
            
            # Get the sharding dimension
            ...
            if shard_dim is not None and mesh is not None:
                # Use manual all_gather to avoid NCCL issues
                world_size = mesh.size()
                gathered_tensors = [torch.empty_like(local_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_tensors, local_tensor, group=mesh.get_group())
                return torch.cat(gathered_tensors, dim=shard_dim)
            else:
                # Fallback: just return local tensor if we can't determine sharding
                return local_tensor
    return param


class FSDPVLLMShardingManager_dypo(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        with simple_timer("reshard", self.timing):
            ...
            if hasattr(peft_model, "peft_config"):
                ...
                # Use standard FSDP state_dict API to avoid DTensor issues
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        # Use the module-level convert_dtensor_to_tensor function
        loaded_params = model.load_weights(
            (
                (name, convert_dtensor_to_tensor(param.to(device, non_blocking=True)))
