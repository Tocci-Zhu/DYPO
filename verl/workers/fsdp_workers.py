...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            # SFT optimizer (separate learning rate)
            sft_lr = optim_config.get("sft_lr", optim_config.lr)  # Default to same as RL if not specified
            ...
            if self.rank == 0:
                ...
                print(f"RL learning rate: {optim_config.lr}")
                print(f"SFT learning rate: {sft_lr}")

            # RL learning rate scheduler
            ...
            # SFT learning rate scheduler (can have different warmup settings)
            sft_warmup_style = optim_config.get("sft_warmup_style", warmup_style)
            sft_num_warmup_steps = int(optim_config.get("sft_lr_warmup_steps", num_warmup_steps))
            if sft_num_warmup_steps < 0:
                sft_warmup_steps_ratio = optim_config.get("sft_lr_warmup_steps_ratio", 0.0)
                sft_num_warmup_steps = int(sft_warmup_steps_ratio * total_steps)

            if sft_warmup_style == "constant":
                sft_lr_scheduler = get_constant_schedule_with_warmup(
                    optimizer=sft_optimizer, num_warmup_steps=sft_num_warmup_steps
                )
            elif sft_warmup_style == "cosine":
                sft_lr_scheduler = get_cosine_schedule_with_warmup(
                    optimizer=sft_optimizer,
                    num_warmup_steps=sft_num_warmup_steps,
                    num_training_steps=total_steps,
                    min_lr_ratio=min_lr_ratio,
                    num_cycles=num_cycles,
                )
            else:
                raise NotImplementedError(f"SFT warmup style {sft_warmup_style} is not supported")
    ...
    def _build_rollout_dypo(self):
        ...
        if self.config.rollout.name == 'hf':
            ...
            from verl.workers.rollout.dypo_rollout_worker import dypo_RolloutWorker
            from verl.workers.sharding_manager.fsdp_vllm import FSDPVLLMShardingManager_dypo
            log_gpu_memory_usage('Before building vllm rollout', logger=None)
        
            self.rollout_config: RolloutConfig = omega_conf_to_dataclass(self.config.rollout)
            self.model_config: HFModelConfig = omega_conf_to_dataclass(self.config.model, dataclass_type=HFModelConfig)
            ...
            rollout = dypo_RolloutWorker(model_path=self.model_config.local_path,
            ...
            if torch.distributed.get_world_size() == 1:
                self.config.rollout.load_format = 'dummy_hf'
            rollout_sharding_manager = FSDPVLLMShardingManager_dypo(module=self.actor_module_fsdp,
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            if self.config.model.sft:
                from verl.workers.actor import DataParallelPPOActor_combined_loss
                self.actor = DataParallelPPOActor_combined_loss(
                    config=actor_cfg, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer
                )
            else:
                self.actor = DataParallelPPOActor(
                    config=actor_cfg, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer, sft_optimizer=self.sft_optimizer
                )

        if self._is_rollout:
            if self.config.model.sft:
                self.rollout, self.rollout_sharding_manager = self._build_rollout_dypo()
            else:
        ...
        if self._is_actor:
            ...
            # Store SFT optimizer and scheduler for separate learning rate management
            self.sft_checkpoint_manager = FSDPCheckpointManager(
                model=self.actor_module_fsdp,
                optimizer=self.sft_optimizer,
                lr_scheduler=self.sft_lr_scheduler,
                processing_class=self.processor if self.processor is not None else self.tokenizer,
                checkpoint_config=self.config.actor.checkpoint,
            )
