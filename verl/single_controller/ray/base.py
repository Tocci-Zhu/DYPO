...
def func_generator(self, method_name, dispatch_fn, collect_fn, execute_fn, blocking):
    class Functor:
        def __call__(this, *args, **kwargs):
            max_retries = 5  # 增加重试次数以应对分布式训练的不稳定性
            retry_count = 0
            ...
            while retry_count < max_retries:
                try:
                    ...
                    retry_count += 1
                    if ("ActorDiedError" in str(e) or "NCCL" in str(e) or "timeout" in str(e).lower()
                        or "ActorUnavailableError" in str(e) or "Socket closed" in str(e)):
                        print(f"[分布式通信错误] 方法 {method_name} 第{retry_count}/{max_retries}次重试")
                        print(f"[错误详情] {type(e).__name__}: {str(e)}")
                        if retry_count < max_retries:
                            import time
                            print(f"[重试等待] 等待5秒后重试...")
                            time.sleep(5)  # 等待5秒后重试
                            continue
                    print(f"[重试失败] 方法 {method_name} 在 {max_retries} 次重试后仍然失败")
                    print(f"[最终错误] {type(e).__name__}: {str(e)}")
                    raise e
