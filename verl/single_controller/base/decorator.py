...
def _split_args_kwargs_data_proto(chunks, *args, **kwargs):
    ...
    splitted_args = []
    for arg in args:
        assert isinstance(arg, (DataProto, DataProtoFuture)), f"arg {arg} of type {type(arg)} is not DataProto or DataProtoFuture"
        splitted_args.append(arg.chunk(chunks=chunks))

    splitted_kwargs = {}
    for key, val in kwargs.items():
        assert isinstance(val, (DataProto, DataProtoFuture)), f"kwarg {key}={val} of type {type(val)} is not DataProto or DataProtoFuture"
